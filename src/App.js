  ...
  const renderContent = (el, index) => {
    switch (el.type) {
      case "headline":
        return (
          <div key={index}>
            <h2 className="w-[100%] text-[8vw] md:text-[2.5vw] mb-[4vh] mt-[8vh] font-playfair text-text">
              {el.content}
            </h2>
          </div>
        );
      case "sec_headline":
        return (
          <h2
            key={index}
            className="md:text-[2vw] text-[6vw] my-[2vh] font-afacad text-text"
          >
            {el.content}
          </h2>
        );
      ...
      case "picture":
        return (
          <div
            key={index}
            className={`w-[100%] h-[20vh] my-[2vh] rounded-sm ${el.content} bg-no-repeat bg-cover bg-center`}
          />
        );
      case "list":
        return (
    ...
    <div className="app">
      ...
      <div className="fixed z-1 bottom-0 w-[100%]">
        <div className="w-[100%] flex flex-row">
          ...
          <CopyToClipboard
            text={JSON.stringify({ [title]: object }, null, 2)}
            onCopy={() => {
              alert("Copied Object!");
              console.log(object);
            }}
      ...
      <div className="flex flex-col w-[100%] lg:px-[30%] p-[7vw] py-[20vh] bg-slate-100 overflow-x-hidden">
        <header>
          <h1 className="w-[100%] text-headline lg:text-[4vw] text-[10vw] font-playfair font-bold">
            {title || "title not yet set"}
          </h1>
          <div className="w-[100%] h-[1px] bg-slate-300 rounded" />
          <div className="flex flex-row justify-between items-center w-[100%] h-[2vh] text-[0.5vw] border-0.5 rounded font-montserrat"></div>
          <hr className="w-[100%] h-[1px] bg-slate-300 rounded" />
        </header>
        {art.map(renderContent)}
      </div>
      <div className="text-slate-300 text-[3vh] font-bold uppercase p-2">Start to type and hit the required button to add to the site, you will see how it will looks like.</div>
    </div>
  );
}
